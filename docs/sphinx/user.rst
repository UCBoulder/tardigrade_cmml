...
.. code:: bash
   ...
   $ abaqus -job <my_input_file> -user ${CONDA_PREFIX}/lib64/tardigrade_balance_equations_umat.o

Where the appropriate path can be confirmed with

.. code:: bash

   $ find ${CONDA_PREFIX} -name "libtardigrade_balance_equations.so"
...
.. code:: bash
   ...
   $ find ${CONDA_PREFIX} -name "tardigrade_balance_equations_umat.o"
   /projects/aea_compute/aea-release/lib64/tardigrade_balance_equations_umat.o
   $ abaqus -job <my_input_file> -user ${CONDA_PREFIX}/lib64/tardigrade_balance_equations_umat.o
...
.. code:: bash
   ...
   $ abaqus -job <my_input_file> -user relative/path/to/tardigrade_balance_equations/build/src/cpp/tardigrade_balance_equations_umat.o
...
``tardigrade_balance_equations/build/_deps/{error,vector,stress,solver,constitutive}_tools-build/``.

.. code:: bash

   $ pwd
   /path/to/my/abaqus/job
   $ cp /path/to/tardigrade_balance_equations/build/src/cpp/{tardigrade_balance_equations_umat.o,libtardigrade_balance_equations.so} .
   $ abaqus -job <my_input_file> -user tardigrade_balance_equations_umat.o
...
tardigrade_balance_equations requires 2 material constants and 2 state variables. The c++ tardigrade_balance_equations interface, material constants, and state
variables are described in the :ref:`sphinx_api`. The fixed expectations for the abaqus interface are defined in the
"Variables" section of the :ref:`sphinx_api` for :ref:`tardigrade_balance_equations_source`. A complete discussion about the constants and their
meaning is not included here. Instead users are directed to calibrated material parameters found in tardigrade_balance_equations entries in the
...
The tardigrade_balance_equations project contains abaqus integration tests for the tardigrade_balance_equations abaqus interface. These tests perform actual abaqus
simulations using the same dummy parameters used for unit and integration testing of the tardigrade_balance_equations c++ code. The tardigrade_balance_equations
Abaqus input files used for integration testing can be found in the tardigrade_balance_equations source code repository with the following bash
command

.. code:: bash

   $ pwd
   /path/to/my/tardigrade_balance_equations
...
.. warning::
   ...
   For calibrated material parameters, see the tardigrade_balance_equations entry for materials found in the `Granta/MIMS`_ `Material
