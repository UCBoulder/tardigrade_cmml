...
.. code:: bash
   ...
   $ abaqus -job <my_input_file> -user ${CONDA_PREFIX}/lib64/tardigrade_cmml_umat.o

Where the appropriate path can be confirmed with

.. code:: bash

   $ find ${CONDA_PREFIX} -name "libtardigrade_cmml.so"
...
.. code:: bash
   ...
   $ find ${CONDA_PREFIX} -name "tardigrade_cmml_umat.o"
   /projects/aea_compute/aea-release/lib64/tardigrade_cmml_umat.o
   $ abaqus -job <my_input_file> -user ${CONDA_PREFIX}/lib64/tardigrade_cmml_umat.o
...
.. code:: bash
   ...
   $ abaqus -job <my_input_file> -user relative/path/to/tardigrade_cmml/build/src/cpp/tardigrade_cmml_umat.o
...
``tardigrade_cmml/build/_deps/{error,vector,stress,solver,constitutive}_tools-build/``.

.. code:: bash

   $ pwd
   /path/to/my/abaqus/job
   $ cp /path/to/tardigrade_cmml/build/src/cpp/{tardigrade_cmml_umat.o,libtardigrade_cmml.so} .
   $ abaqus -job <my_input_file> -user tardigrade_cmml_umat.o
...
tardigrade_cmml requires 2 material constants and 2 state variables. The c++ tardigrade_cmml interface, material constants, and state
variables are described in the :ref:`sphinx_api`. The fixed expectations for the abaqus interface are defined in the
"Variables" section of the :ref:`sphinx_api` for :ref:`tardigrade_cmml_source`. A complete discussion about the constants and their
meaning is not included here. Instead users are directed to calibrated material parameters found in tardigrade_cmml entries in the
...
The tardigrade_cmml project contains abaqus integration tests for the tardigrade_cmml abaqus interface. These tests perform actual abaqus
simulations using the same dummy parameters used for unit and integration testing of the tardigrade_cmml c++ code. The tardigrade_cmml
Abaqus input files used for integration testing can be found in the tardigrade_cmml source code repository with the following bash
command

.. code:: bash

   $ pwd
   /path/to/my/tardigrade_cmml
...
.. warning::
   ...
   For calibrated material parameters, see the tardigrade_cmml entry for materials found in the `Granta/MIMS`_ `Material
