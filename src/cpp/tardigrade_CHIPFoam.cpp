/**
 *****************************************************************************
 * \file tardigrade_CHIPFoam.cpp
 *****************************************************************************
 * A definition of a CHIPFoam hyperelastic material model which can be
    ...
    namespace CHIPFoam {
        ...
        void CHIPFoam::formDeformationGradients(const current_dof_type         *current_dof_begin,
            ...
            tardigradeConstitutiveTools::computeDeformationGradient(
                ...
                deformationGradient, getIsCurrent());

            tardigradeConstitutiveTools::computeDeformationGradient(
                std::vector<previous_dof_type>(previous_dof_begin + getDisplacementGradientIndex(),
                                               previous_dof_begin + getDisplacementGradientIndex() + dim * dim),
                previousDeformationGradient, getIsCurrent());
        }
        ...
        void CHIPFoam::formDeformationGradients(const current_dof_type         *current_dof_begin,
            ...
            tardigradeConstitutiveTools::computeDeformationGradient(
                std::vector<current_dof_type>(current_dof_begin + getDisplacementGradientIndex(),
                                              current_dof_begin + getDisplacementGradientIndex() + dim * dim),
                deformationGradient, dFdGradU, getIsCurrent());
        ...
        int CHIPFoam::evaluate_model(const time_type &current_time, const time_type &dt,
            ...
            try {
                ...
                CHIPFoamHydra hydra(DOFStorage, model_configuration);

                hydra.setTemperatureGradientIndex((unsigned int)(*(parameters_begin + 2) + 0.5));
                hydra.setStressParameters(parameters_begin + 3, parameters_begin + 11);
                hydra.setInternalEnergyParameters(parameters_begin + 11, parameters_begin + 12);
                hydra.setHeatConductionParameters(parameters_begin + 12, parameters_begin + 13);
                ...
                *(result_begin + getInternalEnergyIndex()) = (*hydra.getUnknownVector())[dim * dim];
                ...
                std::copy(std::begin(*hydra.getUnknownVector()) + dim * dim + 1,
        ...
        int CHIPFoam::evaluate_model(const time_type &current_time, const time_type &dt,
            ...
            try {
                ...
                *(result_begin + getInternalEnergyIndex()) = (*hydra.getUnknownVector())[dim * dim];
                ...
                std::copy(std::begin(*hydra.getFlatdXdAdditionalDOF()) + dim * dim * dof_size,
                    ...
                    for (unsigned int j = 0; j < dim * dim; ++j) {
                        for (unsigned int k = 0; k < dim * dim; ++k) {
                            *(jacobian_begin + dof_size * (i + getInternalEnergyIndex()) + k +
                              getDisplacementGradientIndex()) +=
                                (*hydra.getFlatdXdF())[dim * dim * (i + dim * dim) + j] * dFdGradU[dim * j + k];
                        }
                    }
                }
                ...
                for (unsigned int i = 0; i < 1; ++i) {
                    *(jacobian_begin + dof_size * (i + getInternalEnergyIndex()) + getTemperatureIndex()) +=
                        (*hydra.getFlatdXdT())[i + dim * dim];
                }

                // Copy over the Jacobians of the heat flux w.r.t. the additional DOF
                std::copy(std::begin(*hydra.getFlatdXdAdditionalDOF()) + (dim * dim + 1) * dof_size,
                    ...
                    for (unsigned int j = 0; j < dim * dim; ++j) {
                        for (unsigned int k = 0; k < dim * dim; ++k) {
                            *(jacobian_begin + dof_size * (i + getHeatFluxIndex()) + k +
                              getDisplacementGradientIndex()) +=
                                (*hydra.getFlatdXdF())[dim * dim * (i + dim * dim + 1) + j] * dFdGradU[dim * j + k];
                        }
                    }
                }
