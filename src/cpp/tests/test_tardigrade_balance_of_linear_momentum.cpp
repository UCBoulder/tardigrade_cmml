...
{
    0.76, 0.40, 0.08, 0.57, 0.41, 0.37, 0.14, 0.67, 0.44, 0.36, 0.94, 0.39, 0.17, 0.06, 0.99, 0.38, 0.84, 0.08, 0.50, 0.40, 0.27,
    0.74, 0.65, 0.07, 0.29, 0.27, 0.91, 0.23, 0.51, 0.96, 0.99, 0.51, 0.37, 0.45, 0.22, 0.99, 0.32, 0.16, 0.12, 0.87, 0.25, 0.32,
    0.30, 0.75, 0.88, 0.26, 0.14, 0.72, 0.47, 0.09, 0.42, 0.07, 0.98, 0.91, 0.95, 0.86, 0.02, 0.13, 0.67, 0.14, 0.46, 0.20, 0.15,
    0.11, 0.84, 0.84, 0.49, 0.61, 0.23, 0.92, 0.57, 0.64, 0.45, 0.67, 0.65, 0.97, 0.60, 0.17, 0.72, 0.30, 0.23, 0.46, 0.54, 0.04,
    0.59, 0.67, 0.91, 0.54, 0.10, 0.58, 0.86, 0.67, 0.85, 0.14, 0.50, 0.55, 0.84, 0.33, 0.72, 0.60, 0.31, 0.58, 0.83, 0.48, 0.01,
    0.22, 0.41, 0.68, 0.92, 0.36, 0.25, 0.10, 0.73, 0.71, 0.64, 0.81, 0.45, 0.12, 0.33, 0.17, 0.94, 0.90, 0.63, 0.94, 0.95, 0.91,
    0.30, 0.03, 0.52, 0.64, 0.47, 0.98, 0.70, 0.59, 0.78, 0.53, 0.87, 0.47, 0.10, 0.20, 0.45, 0.44, 0.93, 0.43, 0.08, 0.48, 0.71,
    0.23, 0.87, 0.01, 0.70, 0.25, 0.73, 0.61, 0.90, 0.09, 0.49, 0.61, 0.86, 0.02, 0.79, 0.48, 0.90, 0.10, 0.23, 0.49, 0.69, 0.13,
    0.61, 0.44, 0.35, 0.71, 0.57, 0.09, 0.39, 0.04, 0.38, 0.92, 0.63, 0.71, 0.29, 0.68, 0.96, 0.74, 0.86, 0.15, 0.50, 0.20, 0.59,
    0.63, 0.06, 0.98, 0.75, 0.65, 0.33, 0.37, 0.07, 0.12, 0.37, 0.77, 0.73, 0.19, 0.28, 0.06, 0.72, 0.93, 0.43, 0.27, 0.23, 0.78,
    0.65, 0.99, 0.95, 0.07, 0.18, 0.15, 0.50, 0.40, 0.79, 0.20, 0.24, 0.51, 0.59, 0.02, 0.52, 0.24, 0.40, 0.83, 0.33, 0.48, 0.02,
...
BOOST_AUTO_TEST_CASE( test_linearHydraTest, * boost::unit_test::tolerance( DEFAULT_TEST_TOLERANCE ) ){
    ...
    {
        +1.627785800e+00, +1.375788846e+00, -3.826504286e-01, +2.362532305e+00, -1.008295561e+00, +2.148033796e-01,
        +2.817371890e+00, +6.485874666e-01, +3.414395198e-01, +6.728761018e+00, +6.423400169e+00, +5.198637536e+00,
        +4.316364379e+00, -1.579767754e-01, +3.048978055e+00, -1.338036745e+00, +2.161553006e+00, +1.798628395e+00,
        +2.927835129e+00, +2.287465062e+00, +4.510422901e-01, +4.493259271e-01
    };
...
//    std::cout << "density_tp1_p     : "; for ( auto v = std::begin( density_tp1_p     ); v != std::end( density_tp1_p     ); ++v ){ std::cout << *v << " "; } std::cout << "\n";
//    std::cout << "density_dot_tp1_p : "; for ( auto v = std::begin( density_dot_tp1_p ); v != std::end( density_dot_tp1_p ); ++v ){ std::cout << *v << " "; } std::cout << "\n";
//    std::cout << "graD_density_tp1  : "; for ( auto v = std::begin( grad_density_tp1  ); v != std::end( grad_density_tp1  ); ++v ){ std::cout << *v << " "; } std::cout << "\n";
//    std::cout << "v_tp1_p           : "; for ( auto v = std::begin( v_tp1_p );           v != std::end( v_tp1_p           ); ++v ){ std::cout << *v << " "; } std::cout << "\n";
//    std::cout << "a_tp1_p           : "; for ( auto v = std::begin( a_tp1_p );           v != std::end( a_tp1_p           ); ++v ){ std::cout << *v << " "; } std::cout << "\n";
...
//                }
    ...
    for ( unsigned int _i = 0; _i < _rows; ++_i ){
        for ( unsigned int _j = 0; _j < _cols; ++_j ){
            std::cout << *( dRdE_begin + _cols * _i + _j ) << " ";
        } std::cout << "\n";
    }
...
BOOST_AUTO_TEST_CASE( test_computeBalanceOfLinearMomentum_hydra_fea, * boost::unit_test::tolerance( 1e-5 ) ){
    ...
    std::array< floatType, 8 * nphases * 3 > answer = {
        0.11012336,  0.1081528 ,  0.07207216,  0.17148693,  0.15922128,
        0.10177719,  0.14472238,  0.14251238,  0.08994445,  0.17877274,
        0.17273578,  0.11067094,  0.0759665 ,  0.06542467,  0.04418563,
        0.11761029,  0.09449228,  0.05988231,  0.1000611 ,  0.08568833,
        0.05451673,  0.1222498 ,  0.10259535,  0.06509041,  0.05629196,
        0.04629866,  0.02904898,  0.08651784,  0.06552228,  0.0376922 ,
        0.073903  ,  0.06018115,  0.0346588 ,  0.08958138,  0.07106539,
        0.04086439,  0.08200687,  0.07862463,  0.04921386,  0.12690808,
        0.1141865 ,  0.06776703,  0.10740665,  0.10305557,  0.05992474,
        0.13186068,  0.1237579 ,  0.07354846,  0.18854373,  0.43785992,
        0.35082077,  0.24345158,  0.45226675,  0.3396839 ,  0.17772447,
        0.51590274,  0.38416765,  0.18937974,  0.49344263,  0.39114012,
        0.0898755 ,  0.1436669 ,  0.14633935,  0.09437847,  0.04784891,
        0.05985999,  0.07079791,  0.13815352,  0.13537054,  0.04708086,
        0.05505737,  0.08042863, -0.01709093,  0.01010829,  0.01708847,
       -0.07226441, -0.13621821, -0.11465524, -0.06528576, -0.03449562,
       -0.03264183, -0.12219918, -0.14636994, -0.11336054,  0.02906032,
        0.21149162,  0.14600306, -0.00725485,  0.13045488,  0.05331236,
       -0.0248849 ,  0.22054529,  0.1171856 , -0.06698132,  0.14217525,
        0.07407468
    };
    ...
    // Check the derivatives w.r.t. the internal energy
    {
        ...
        for ( unsigned int i = 0; i < vardim; ++i ){
            ...
            for ( unsigned int j = 0; j < outdim; ++j ){
                ...
                std::cout << "j, node, phase, res, ans: " << j << ", " << node << ", " << phase << ", " << dRdE[ vardim * j + i ] << ", " << ( vp[ j ] - vm[ j ] ) / ( 2 * delta ) << "\n";
