...
BOOST_AUTO_TEST_CASE(test_LinearTestMaterial, *boost::unit_test::tolerance(DEFAULT_TEST_TOLERANCE)) {
    ...
    int error_code = model.evaluate_model(current_time, dt, dof.data(), previous_dof.data(), dof.size(),
                                          parameters.data(), parameters.size(), sdvs.data(), sdvs.size(),
                                          result.data(), result.size(), output_message);

    if (error_code != 0){std::cerr << output_message << "\n"; BOOST_CHECK(false);}

    BOOST_TEST(answer == result, CHECK_PER_ELEMENT);

    error_code = model_J.evaluate_model(current_time, dt, dof.data(), previous_dof.data(), dof.size(),
                                        parameters.data(), parameters.size(), sdvs.data(), sdvs.size(),
                                        result_J.data(), result_J.size(), jacobian.data(),
                                        additional.data(), additional.size(), output_message);
    ...
    double eps = 1e-6;
    {
        ...
        for (unsigned int i = 0; i < vardim; ++i) {
            ...
            int ep = model.evaluate_model(current_time, dt, xp.data(), previous_dof.data(), dof.size(),
                                          parameters.data(), parameters.size(), sdvs.data(), sdvs.size(),
                                          rp.data(), rp.size(), output_message);
            int em = model.evaluate_model(current_time, dt, xm.data(), previous_dof.data(), dof.size(),
                                          parameters.data(), parameters.size(), sdvs.data(), sdvs.size(),
                                          rm.data(), rm.size(), output_message);

            if (ep != 0){BOOST_CHECK(false);}
            if (em != 0){BOOST_CHECK(false);}
