        ...
        class definedPlasticEvolutionHydra : public tardigradeCMML::BasicSolid::basicSolidHydra {
           public:
            ...
            void setDefinedVelocityGradientIndex(const unsigned int &value) {
                /*!
                 * Set the starting index in the additional degrees of freedom of the additional velocity gradient that
                 * defines the permanent deformation
                 *
                 * \param &value: The value of the velocity gradient index
                 */
            ...
            void setDensityGradientIndex(const unsigned int &value) {
                /*!
                 * Set the starting index in the additional degrees of freedom of the density gradient that defines the
                 * mass diffusion
                 *
            ...
            const unsigned int getDOFInternalEnergyIndex() {
                ...
                TARDIGRADE_ERROR_TOOLS_CHECK(_dof_internal_energy_index_set,
                                             "The internal energy index must be set before it is called");
            ...
            const unsigned int getDefinedVelocityGradientIndex() {
                /*!
                 * Get the starting index of the velocity gradient that defines the permanent deformation in the
                 * additional degree of freedom vector
                 */

                TARDIGRADE_ERROR_TOOLS_CHECK(_defined_velocity_gradient_index_set,
                                             "The defined velocity gradient index must be set before it is called");
            ...
            const bool getInternalEnergyScaledByDensity() {
                ...
                TARDIGRADE_ERROR_TOOLS_CHECK(_internal_energy_scaled_by_density_set,
                                             "The flag for whether the internal energy is scaled by the density must "
            ...
            const unsigned int getDensityGradientIndex() {
                ...
                TARDIGRADE_ERROR_TOOLS_CHECK(_density_gradient_index_set,
                                             "The density gradient index must be set before it is called");
            ...
            virtual void setResidualClasses() override {
                ...
                defined_deformation = tardigradeHydra::dofVelocityGradientDeformation::residual(
                    this, getStressSize() + 2, 1, getDensityIndex(), getDOFInternalEnergyIndex(),
                    getDefinedVelocityGradientIndex(), getInternalEnergyScaledByDensity(), {7, 8},
                    *getDefinedDeformationParameters(), getIntegrationParameter());
                defined_deformation.setUseTrapezoidalIntegration(true);

                internal_energy =
                    tardigradeHydra::linearInternalEnergy::residual(this, getInternalEnergySize(),
                                                                    *getInternalEnergyParameters(), 1, 18);

                heat_conduction =
                    tardigradeHydra::fourierHeatConduction::residual(this, getHeatConductionSize(),
                                                                     *getHeatConductionParameters(),
                                                                     getTemperatureGradientIndex(), 1, 19);

                mass_diffusion = tardigradeHydra::fourierHeatConduction::residual(this, getHeatConductionSize(),
                                                                                  *getMassDiffusionParameters(),
                                                                                  getDensityGradientIndex(), 1, 22);
            ...
            const std::vector<double> *getDefinedDeformationParameters() { /*! Get the defined deformation parameters */
                return &_defined_deformation_parameters;
            }

            const std::vector<double> *getMassDiffusionParameters() { /*! Get the mass diffusion parameters */
                return &_mass_diffusion_parameters;
            }

            const double getIntegrationParameter() { /*! Get the integration parameter */
                return _integration_parameter;
            }

           protected:
            tardigradeHydra::fourierHeatConduction::residual
                mass_diffusion;  //!< The residual that defines the mass diffusion (mathematically the same as Fourier
                                 //!< heat conduction with a density gradient)
            tardigradeHydra::dofVelocityGradientDeformation::residual
                defined_deformation;  //!< The residual that defines the externally defined permanent deformation

           private:
            double _integration_parameter =
                1.0;  //!< The integration parameter for the defined deformation (0 is explicit, 1 is implicit)
            std::vector<double> _defined_deformation_parameters;  //!< The parameters associated with the calculation of
                                                                  //!< the defined deformation
            ...
            unsigned int
                 _dof_internal_energy_index;  //!< The index in the additional DOF vector which is the internal energy
            bool _dof_internal_energy_index_set = false;    //!< The flag for if _dof_internal_energy_index is set
            unsigned int _defined_velocity_gradient_index;  //!< The index in the additional DOF vector that the defined
                                                            //!< velocity gradient is defined in
            bool _defined_velocity_gradient_index_set =
                false;                                //!< The flag for if _defined_velocity_gradient_index is set
            bool _internal_energy_scaled_by_density;  //!< The flag for if the internal energy is scaled by the density
            bool _internal_energy_scaled_by_density_set =
                false;                                 //!< The flag for if _internal_energy_scaled_by_density is set
            unsigned int _density_gradient_index;      //!< The index in the additional DOF vector that the denensity
                                                       //!< gradient is defined in
        ...
        class DefinedPlasticEvolution : public tardigradeCMML::BasicSolid::BasicSolid {
           public:
            ...
            void setDefinedVelocityGradientIndex(unsigned int value) {
                /*!
                 * Set the index the velocity gradient that defines the permanent deformation is located in the dof
                 * vector
                 *
            ...
            const unsigned int getDOFInternalEnergyIndex() { /*! Get the internal energy index */
                return _dof_internal_energy_index;
            }

            const unsigned int getDefinedVelocityGradientIndex() { /*! Get the defined velocity gradient index */
                return _defined_velocity_gradient_index;
            }

            const bool getInternalEnergyScaledByDensity() { /*! Get the internal energy index */
                return _internal_energy_scaled_by_density;
            }

            const unsigned int getDensityGradientIndex() { /*! Get the density gradient index */
                return _density_gradient_index;
            }

            const double getMassDiffusionCoefficient() { /*! Get the mass diffusion coefficient */
                return _mass_diffusion_coefficient;
            }
           ...
           protected:
            virtual void extract_parameters(const parameter_type *parameters_begin,
                                            const unsigned int    parameters_size) override {
                /*!
                 * Extract the material parameters
                 *
                 * The parameter vector is assumed to be organized as
                 *
                 * defined velocity gradient index, density gradient index, displacement gradient index, temperature
                 * index, temperature gradient index, lambda, mu, specifc heat, conductivity, mass diffusion coefficient
                 *
           ...
           private:
            ...
            unsigned int
                _dof_internal_energy_index;  //!< The index of the internal energy that defines the mass change rate

            unsigned int _defined_velocity_gradient_index;  //!< The index of the velocity gradient that defines the
                                                            //!< deformation evolution

            bool _internal_energy_scaled_by_density;  //!< Whether the internal energy is scaled by the density

            unsigned int
                _density_gradient_index;  //!< The index of the density gradient that defines the mass diffusion
            ...
            unsigned int _internal_heat_generation_rate_index =
                20;  //!< The index of the internal heat generation rate in the result vector
