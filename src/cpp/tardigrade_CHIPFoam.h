/*!
=====================================================================
|                      tardigrade_CHIPFoam.h                        |
=====================================================================
| A model for elastomeric foams using the CHIPFoam strain energy    |
| potential (Lewis, M., A Robust, Compressible, Hyperelastic        |
| Constitutive Model for the Mechanical Response of Foamed Rubber,  |
| 2016)                                                             |
---------------------------------------------------------------------
| The output material vector is comprised of the following          |
| responses.                                                        |
|                                                                   |
| Stress response          : CHIPFoam Hyperelasticity               |
...
#ifndef TARDIGRADE_CHIPFOAM_H
#define TARDIGRADE_CHIPFOAM_H
...
namespace tardigradeCMML {
    ...
    namespace CHIPFoam {
        ...
        /*!
         * The CHIPFoam material model hydra class
         */
        class CHIPFoamHydra : public tardigradeHydra::hydraBase {
           public:
            ...
            const std::vector<double> *getStressParameters() { /*! Get the stress parameters */
                return &_stress_parameters;
            }

            const std::vector<double> *getInternalEnergyParameters() { /*! Get the internal energy parameters */
                return &_internal_energy_parameters;
            }

            const std::vector<double> *getHeatConductionParameters() { /*! Get the heat conduction parameters */
                return &_heat_conduction_parameters;
            }

            const unsigned int getStressSize() { /*! Get the number of nonlinear unknowns in the stress response */
                return _stress_size;
            }

            const unsigned int
            getInternalEnergySize() { /*! Get the number of nonlinear unknowns in the internal energy response */
                return _internal_energy_size;
            }

            const unsigned int
            getHeatConductionSize() { /*! Get the number of nonlinear unknowns in the heat conduction response */
           ...
           private:
            unsigned int _stress_size =
                9;  //!< The number of nonlinear unknowns in the stress response (must be at least 9)
            unsigned int _internal_energy_size =
                1;  //!< The number of nonlinear unknowns in the internal energy response (must be at least 1)
            unsigned int _heat_conduction_size =
                3;  //!< The number of nonlinear unknowns in the heat conduction response (must be at least 3)
            std::vector<double> _stress_parameters;  //!< The parameters associated with the calculation of the stress
            std::vector<double>
                _internal_energy_parameters;  //!< The parameters associated with the calculation of the internal energy
            std::vector<double>
                _heat_conduction_parameters;  //!< The parameters associated with the calculation of the heat conduction
        ...
        /*!
         * A CHIPFoam hyperelastic solid
         */
        class CHIPFoam : public tardigradeCMML::CMMLMaterial {
           public:
            CHIPFoam() {
                /*!
            ...
            const bool getIsCurrent() { /*! Get if the dof gradients are with respect to the current configuration */
                return _isCurrent;
            }
            const unsigned int getDisplacementGradientIndex() { /*! Get the displacement gradient index */
            ...
            const unsigned int getStressIndex() { /*! Get the stress index in the material response vector */
                return _stress_index;
            }
            const unsigned int
            getInternalEnergyIndex() { /*! Get the internal energy index in the material response vector */
                return _internal_energy_index;
            }
            const unsigned int getHeatFluxIndex() { /*! Get the heat flux index in the material response vector */
           ...
           protected:
            ...
            virtual void extract_parameters(const parameter_type *parameters_begin,
                ...
                TARDIGRADE_ERROR_TOOLS_CHECK(parameters_size == 13, "The parameters vector must have seven values")
           ...
           private:
            ...
            unsigned int _internal_energy_index =
                9;                               //!< The index of the internal energy in the material response vector
        ...
        REGISTER_MATERIAL(tardigradeCMML::CHIPFoam::CHIPFoam);

    }  // namespace CHIPFoam

}  // namespace tardigradeCMML

#include "tardigrade_CHIPFoam.cpp"
